...
import per.piers.onlineJudge.service.TokenService;
...
public class UserController {
    ...
    public String register(String email, HttpServletRequest request) throws MessagingException {
        String key = TokenService.addURLToken(System.currentTimeMillis(), email);
    ...
    public String register(@PathVariable("token") String token, String email, String password, String name, String sex, Boolean enabled, String role) {
        if (Objects.equals(TokenService.getEmailFromToken(token), email)) {
            ...
            int line = dao.insertUser(user);
            if (line == 0) throw new CRUDException("insert failed: " + user);
            else if (line > 1) throw new CRUDException("insert size > 1" + user);
            else return "SUCCESS";
        } else {
